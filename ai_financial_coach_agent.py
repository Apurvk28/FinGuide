...
class InvestmentRecommendation(BaseModel):
    instrument: str = Field(..., description="Investment instrument (e.g., Mutual Funds, FDs, PPF, Stocks)")
    allocation_percentage: float = Field(..., description="Percentage of surplus to allocate")
    rationale: str = Field(..., description="Why this instrument is recommended for the Indian market")

class InvestmentStrategy(BaseModel):
    risk_profile: str = Field(..., description="Assessed risk profile (Conservative, Moderate, Aggressive)")
    monthly_surplus_allocation: float = Field(..., description="Total amount to invest monthly")
    recommendations: List[InvestmentRecommendation] = Field(..., description="Specific investment recommendations")
...
IMPORTANT: Store your final plan in state['debt_reduction'] and ensure it aligns with the previous analyses.""",
        ...
        self.investment_strategy_agent = LlmAgent(
            name="InvestmentStrategyAgent",
            model="gemini-2.5-flash",
            description="Recommends investment instruments specifically tailored for the Indian context based on monthly surplus",
            instruction="""You are an Investment Strategy Agent specializing in the Indian financial market.
You are the final agent in the sequence. READ the budget analysis, savings strategy, and debt reduction state first.

Your tasks:
1. Calculate the real monthly surplus from the budget analysis (Income - Expenses) minus any new recommended debt or savings commitments.
2. Assess risk profile based on age (assume average if not provided), dependants, and existing safety nets (emergency fund).
3. Recommend specific Indian investment vehicles:
   - Mutual Funds (Equity/Debt SIPs)
   - Fixed Deposits (FDs)
   - Public Provident Fund (PPF)
   - Direct Indian Equities
4. Provide a percentage allocation that totals 100%.

IMPORTANT: Store your strategy in state['investment_strategy'].""",
            output_schema=InvestmentStrategy,
        ...
        self.coordinator_agent = SequentialAgent(
            ...
            sub_agents=[
                ...
                self.investment_strategy_agent
            ]
        )
    ...
    async def analyze_finances(self, financial_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            results = {}
            for key in ["budget_analysis", "savings_strategy", "debt_reduction", "investment_strategy"]:
    ...
    def _create_default_results(self, financial_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "investment_strategy": {
                ...
                "monthly_surplus_allocation": 0,
                "recommendations": [
                    {"instrument": "Index Mutual Funds", "allocation_percentage": 50, "rationale": "Long term wealth creation"},
                    {"instrument": "Fixed Deposits (FD)", "allocation_percentage": 50, "rationale": "Capital safety"}
                ]
            }
        }
    ...
    if "recommendations" in analysis:
        ...
        for rec in analysis["recommendations"]:
            ...
            if "potential_savings" in rec:
                st.metric(f"Potential Monthly Savings", f"₹{rec['potential_savings']:,}")
...
def display_savings_strategy(strategy: Dict[str, Any]):
    ...
    if "emergency_fund" in strategy:
        ...
        st.markdown(f"**Recommended Size**: ₹{ef['recommended_amount']:,}")
        ...
        if "current_amount" in ef and "recommended_amount" in ef:
            ...
            st.markdown(f"₹{ef['current_amount']:,} of ₹{ef['recommended_amount']:,}")
    
    if "recommendations" in strategy:
        st.markdown("### Recommended Savings Allocations")
        for rec in strategy["recommendations"]:
            st.markdown(f"**{rec['category']}**: ₹{rec['amount']:,}/month")
...
def display_debt_reduction(plan: Dict[str, Any]):
    ...
    if "total_debt" in plan:
        st.metric("Total Debt", f"₹{plan['total_debt']:,}")
    
    if "debts" in plan and isinstance(plan["debts"], list) and len(plan["debts"]) > 0:
...
def display_investment_strategy(strategy: Dict[str, Any]):
    if isinstance(strategy, str):
        try:
            strategy = json.loads(strategy)
        except json.JSONDecodeError:
            st.error("Failed to parse investment strategy results")
            return
    if not isinstance(strategy, dict):
        st.error("Invalid investment strategy format")
        return
        
    st.subheader(f"Investment Strategy (Risk Profile: {strategy.get('risk_profile', 'Moderate')})")
    surplus = strategy.get('monthly_surplus_allocation', 0)
    st.metric("Amount to Invest Monthly", f"₹{surplus:,}")
    
    if "recommendations" in strategy:
        st.markdown("### Recommended Instruments (Indian Context)")
        for rec in strategy["recommendations"]:
            amt = (rec['allocation_percentage'] / 100) * surplus
            st.markdown(f"**{rec['instrument']}** - {rec['allocation_percentage']}% (₹{amt:,})")
            st.markdown(f"_{rec['rationale']}_")


def generate_docx_report(results) -> io.BytesIO:
    doc = Document()
    doc.add_heading('FinGuide Financial Analysis Report', 0)
    
    # 1. Budget Analysis
    if "budget_analysis" in results and results["budget_analysis"]:
        ba = results["budget_analysis"]
        if not isinstance(ba, str):
            doc.add_heading('Budget Analysis', level=1)
            if "total_expenses" in ba:
                doc.add_paragraph(f"Total Expenses: ₹{ba['total_expenses']:,}")
            if "insights" in ba:
    ...
    if "savings_strategy" in results and results["savings_strategy"]:
        ...
        if not isinstance(ss, str):
            doc.add_heading('Savings Strategy', level=1)
            if "emergency_fund_target" in ss:
                doc.add_paragraph(f"Emergency Fund Target: ₹{ss['emergency_fund_target']:,}")
            if "savings_rate" in ss:
                doc.add_paragraph(f"Recommended Savings Rate: {ss['savings_rate']}%")
    ...
    if "debt_reduction" in results and results["debt_reduction"]:
        ...
        if not isinstance(dr, str):
            doc.add_heading('Debt Reduction Plan', level=1)
            if "total_debt" in dr:
                doc.add_paragraph(f"Total Debt: ₹{dr['total_debt']:,}")
            if "recommendations" in dr:
                doc.add_heading('Recommendations', level=2)
                for rec in dr["recommendations"]:
                    doc.add_paragraph(f"• {rec.get('title', '')}: {rec.get('description', '')}")

    # 4. Investment Strategy
    if "investment_strategy" in results and results["investment_strategy"]:
        iv = results["investment_strategy"]
        if not isinstance(iv, str):
            doc.add_heading('Investment Strategy (India)', level=1)
            if "risk_profile" in iv:
                doc.add_paragraph(f"Risk Profile: {iv['risk_profile']}")
            if "monthly_surplus_allocation" in iv:
                doc.add_paragraph(f"Monthly Surplus Allocation: ₹{iv['monthly_surplus_allocation']:,}")
            if "recommendations" in iv:
                doc.add_heading('Recommended Instruments', level=2)
                for rec in iv["recommendations"]:
                    doc.add_paragraph(f"• {rec.get('instrument', '')} ({rec.get('allocation_percentage', 0)}%): {rec.get('rationale', '')}")
...
def main():
    ...
    if True:
        ...
        with st.container():
            ...
            # Define default expense categories with emojis
            categories = [
                ("🏠 Housing", "Housing"),
                ("🔌 Utilities", "Utilities"),
                ("🍽️ Food", "Food"),
            ...
            for cat_name in st.session_state.custom_categories:
                categories.append((f"🏷️ {cat_name}", cat_name))
            ...
            with st.expander("➕ Add New Category"):
                ...
                if st.button("Add Category"):
                    if new_cat_name and new_cat_name not in st.session_state.custom_categories:
                        st.session_state.custom_categories.append(new_cat_name)
                        st.rerun()
        ...
        if analyze_button:
            ...
            with st.spinner("🤖 AI agents are analyzing your financial data..."): 
                ...
                try:
                    ...
                    with tabs[3]:
                        st.subheader("Investment Strategy (India)")
                        if "investment_strategy" in results and results["investment_strategy"]:
                            display_investment_strategy(results["investment_strategy"])
                        else:
                            st.write("No investment strategy available.")
                except Exception as e:
